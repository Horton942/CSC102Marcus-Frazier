...
function playCraps(){
    // we are going to narrate what the finction does through the console
    ...
    console.log("The first die roll is: " + die1);
     ...
     // let's code the rules of the dice gmae
    ...
    // if neither of the previous if, or else if the statements were true, then this catch-all
...
function palindromeFun(){
    ...
    do{
        ...
        if (answer == "n"){
            // set our boolean loop monitor variable to false so that the loop will stop runninf when the while condition is tested below
            bContinue = false;
        }
    }
    while(bContinue); //while bcontinue is true, the loop will keep running; ic bContinue is false, the loop will not run anymore
}
...
/* Sound code */
function addSound(){
    // create an audion HTML element in JavaScript that  we will add to the HTML pahe
    ...
    soundElem.setAttribute("id", "calmingStorm");
    // specify the sound file
    soundElem.setAttribute("src", "mixkit-calm-thunderstorm-in-the-jungle-2415.wav");
...
function playSound(){
    ...
    var audio = document.getElementById("calmingStorm");
    // play the sound
    audio.play();
...
// Week 12 Array code for plants and video games
 ...
 function showPlants(){
    ...
    // i is the  counter variable; the loop should run as loing as i is less than the number of items in the array; increasethe value of i by 1 each time the loop runs
...
function showStats(){
    ...
    // set up a pointer variable to our stats div
    ...
    for (var i=0; i<gameNameArr.length; i++){
        ...
        //add the name of the stat to the text of the list item
    ...
    // hide the show Stats button
    statsButton.remove();
}

// Week 12 Lap Time Recorder
// this variable will keep track of the current lap
        // as we add laps, rhe value will be increased by 1
        var lapCounter = 0;
        ...
        function displayLaps(){
            ...
            // loop throught the laps array to add the lap times to the unordered list
